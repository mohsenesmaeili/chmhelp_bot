function run(msg)
	...
	about_key = {{{text = "کانال رسمی" , url = "https://telegram.me/FullManager"}},{{text = "ربات تیم" , url = "https://telegram.me/chmhelp_bot"}},{{text = "پیام رسان" , url = "https://telegram.me/FullManagerBot"}}}
	start_txt = "به ربات "..bot.first_name..' خوش آمدید\nاین ربات در حالت اینلاین رایگان است و برای آموزش بیشتر به بخش "راهنمای استفاده رایگان" مراجعه کنید.\n`استفاده ی وی آی پی از این ربات رایگان نبوده و جهت تهیه ی اکانت از طریق پل های ارتباطی موجود در بخش "ارتباط با ما" اقدام نمایید و با ما تماس بگیرید. هزینه ی استفاده از این ربات ماهانه فقط 1.000 تومان میباشد.\nاز جمله قابلیات این ربات میتوان به موارد زیر اشاره کرد:\n-ارسال متن به کانال از طریق پیامک (اس ام اس)\n-ارسال کیبرد شیشه ای به کانال\n-افزودن، ویرایش و حذف درج نویس در فورواردها\n-حذف منبع فورواردها\n-ارسال زماندار پست\n-ارسال متون با استایل و فونت های مختلف\n-افزودن لینک به یک کلمه\n-و...`'
...
`با استفاده از این کلید میتوانید نام کانال، یوزرنیم کانال، آی دی کانال، شماره ارتباطی با سامانه پیامکی، انقضای اکانت، آمار کانال، سازنده ی کانال، تعداد ادمینهای کانال، لیست ادمینهای کانال را در یک لیست ببینید.`]]
	...
	elseif msg.text == "مثال کیبرد اینلاین و مارک داون" then
		inlinekey = {{{text = "کانال تیم" , url = "https://telegram.me/FullManager"}},{{text = "ربات تیم" , url = "https://telegram.me/chmhelp_bot"}}}
		text = "مثال استایل نویس با فونتهای مختلف:\n*MDesign Team*\nMDesign Team\n_MDesign Team_\n`MDesign Team`\nتیم ربات\n`تیم رباتساز`\nمثال هایپر لینک:\n[تیم رباتساز](telegram.me/FullManager)\n[MDesign Team](telegram.me/FullManager)\nمثال کیبرد اینلاین:"
	...
	if msg.text:find('/add') and msg.from.id == sudo_id then
		...
		if usertarget then
			...
			return send_msg(sudo_id, "*/add 12345678,@FullManager,9131234567*\n`/add [telegram id],[@channel],[number]`", true)
		end
	elseif msg.text:find('/edit') and msg.from.id == sudo_id then
		...
		if usertarget then
			...
			return send_msg(sudo_id, "*/edit 12345678,@FullManager,9131234567*\n`/edit [telegram id],[@channel],[number]`", true)
		end
	...
	elseif msg.text == "مثال کیبرد اینلاین" then
		return send_inline(msg.from.id, "`تیتر کیبرد اینلاین با قابلیت استفاده از قابلیت مارک داون`", {{{text = "کانال تیم" , url = "https://telegram.me/FullManager"}}})
	end
